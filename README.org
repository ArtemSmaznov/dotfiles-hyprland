...
bind =       , q , exec   , $myEwwScript/toggle-window.sh -m $($myScript/get-current-monitor.sh id) $status-panel # toggle status panel
bind = SHIFT , q , exec   , $myEwwScript/toggle-window.sh $status-panel                                           # toggle status panel on default monitor
