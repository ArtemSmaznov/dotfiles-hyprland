...
exec-once = uwsm app -S both -- eww open-many main-bar second-bar $eww-indicators $eww-quick-settings
...
workspace = 9  , persistent:false , monitor:DP-1 , gapsin:100 , gapsout:200 , on-created-empty: $myCliMusic
windowrule = border_size 3, match:workspace 9
workspace = 10 , persistent:false , monitor:DP-3 , default:true , gapsin:0 , gapsout:0 , rounding:0 , on-created-empty: $mySysLog
windowrule = border_size 2, match:workspace 10
#+end_src
...
# config
layerrule = blur true, match:namespace $eww-config
layerrule = ignore_alpha 0, match:namespace $eww-config
layerrule = animation slide, match:namespace $eww-config
...
animations {
    ...
    animation = windowsMove         , 1 , 10 , overshoot
...
bindd = SUPER SHIFT , m   , mirror layout          , layoutmsg       , togglesplit
#+end_src
...
bindd = SUPER , 0   , switch to workspace 10 , focusworkspaceoncurrentmonitor , 10
#+end_src
...
bindd = SUPER SHIFT , 0 , move window to workspace 10 , movetoworkspacesilent , 10
#+end_src
...
bindd = SUPER SHIFT CTRL , 0 , move window to workspace 10 with switch , moveworkspacetomonitor , 10 current
...
bindd  =       , v , volume control                    , exec   , $myVolumeControl
...
submap = leader
bind   =       , c , exec                    , $myEwwScript/which-key.sh -p "SPC c-" config
bindd  =       , c , +config                 , submap , config
...
#+begin_src conf
submap = config
bind  = , a , exec                , $myEwwScript/which-key.sh -p "SPC c a-" autostart
bindd = , a , open autostart list , exec , eww open $eww-autostart
bind  = , a , submap              , autostart

bindr = , catchall  , exec   , $myHyprScript/reset-submap.sh
bind  = , backspace , exec   , $myEwwScript/which-key.sh -p "SPC-" leader
bind  = , backspace , submap , leader
bindr = , a         , exec   , $myHyprScript/reset-submap.sh
...
#+begin_src conf
submap = autostart
bindd = , b , toggle bluetooth   , exec , $exile autostart toggle blueman-applet       && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , d , toggle dunst       , exec , $exile autostart toggle dunst                && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , e , toggle emacs       , exec , $exile autostart toggle emacs                && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , k , toggle kdeconnect  , exec , $exile autostart toggle kdeconnect-indicator && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , l , toggle lutris      , exec , $exile autostart toggle lutris               && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , n , toggle nextcloud   , exec , $exile autostart toggle nextcloud            && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , q , toggle browser     , exec , $exile autostart toggle qutebrowser          && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , s , toggle steam       , exec , $exile autostart toggle steam                && eww update autostart-enabled-apps=$($exile autostart list)
bindd = , t , toggle thunderbird , exec , $exile autostart toggle thunderbird          && eww update autostart-enabled-apps=$($exile autostart list)

bindr = , catchall  , exec   , $myHyprScript/reset-submap.sh && eww close $eww-autostart
bind  = , backspace , exec   , $myEwwScript/which-key.sh -p "SPC c-" config
bind  = , backspace , exec   , eww close $eww-autostart
bind  = , backspace , submap , config
...
bindd  = , s , scrolling     , exec   , $myHyprScript/layoutctl.sh set scrolling
