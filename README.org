...
exec-once = uwsm app -S both -- eww open-many main-bar second-bar $quick-settings indicators
exec-once = dunst
#+end_src
...
bindd  =       , p , playground panel                          , exec , $myEwwScript/toggle-window.sh -m $($myScript/get-current-monitor.sh id) $playground-eww
bindd  = SHIFT , P , playground panel on secondary monitor     , exec , $myEwwScript/toggle-window.sh -m 1 $playground-eww
